...
class Config(object):
    TG_BOT_TOKEN = os.environ.get("TG_BOT_TOKEN", "8516803336:AAFrW5USktHv30_12HsmHriYU9oglDkj8j4") # Make a bot from https://t.me/BotFather and enter the token here
    #If deploying on vps edit the above value as example := TG_BOT_TOKEN = "Your-bot-token-inside-inverted-commas."
    
    APP_ID = int(os.environ.get("29232353", 12345)) # Get this value from https://my.telegram.org/apps
    #If deploying on vps edit the above value as example := APP_ID = Your-APP_ID-without-inverted-commas
    
    API_HASH = os.environ.get("6868788228291767c90e4346eea03f36") # Get this value from https://my.telegram.org/apps
    ...
    Bot_username = os.environ.get("Bot_username", "@Megadownlodd_bot") # Your bot's telegram username (must enter with '@' in the front of the username)
    #If deploying on vps edit the above value as example := Bot_username = "Your-Bot_username-inside-inverted-commas."
    
    OWNER_ID = os.environ.get("6650855788", None) # Your(owner's) telegram id
